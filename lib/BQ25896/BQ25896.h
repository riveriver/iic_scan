...
class BQ25896 {
 public:
  enum class TS_RANK { NORMAL = 0, WARM = 2, COOL = 3, COLD = 5, HOT = 6 };
  ...
  enum class VBUS_STAT { NO_INPUT = 0, USB_HOST = 1, ADAPTER = 2, OTG = 7 };
  ...
  enum class VSYS_STAT { NOT_IN_VSYS = 0, IN_VSYSMIN = 1 };

  enum class BOOST_FAULT {
    NORMAL = 0,
    FAULT  = 1,
  };
  enum class BAT_FAULT {
    NORMAL = 0,
    BATOVP  = 1,
  };
  enum class  WATCHDOG_FAULT {
    NORMAL = 0,
    TIMER_EXPIRATION = 1,
  };
  enum class CHRG_FAULT {
    NORMAL = 0,
    INPUT_FAULT = 1,
    THERMAL_SHUTDOWN = 2,
    TIMER_EXPIRATION = 3
 ...
 private:
  ...
  TwoWire* wire;
  float VBUS, VSYS, VBAT, ICHG, TSPCT, VINDPM, Temperature;
  ...
  void takeFaultBQ25896(void) {
    uint8_t data = read(REG::FAULT_);
    if (((data) >> (3)) & 0x01){
        BAT_FAULT_ = BAT_FAULT::BATOVP;
    }else{
        BAT_FAULT_ = BAT_FAULT::NORMAL;
    }
    ...
    } else  // 0
    {
     ...
     // parsing boost fault
    if (((data) >> (6)) & 0x01){
        BOOST_FAULT_ = BOOST_FAULT::FAULT;
    }else{
        BOOST_FAULT_ = BOOST_FAULT::NORMAL;
    }
     // parsing boost fault
    if (((data) >> (7)) & 0x01){
        WATCHDOG_FAULT_ = WATCHDOG_FAULT::TIMER_EXPIRATION;
    }else{
        WATCHDOG_FAULT_ = WATCHDOG_FAULT::NORMAL;
    }
  }
 ...
 public:
  ...
  BOOST_FAULT BOOST_FAULT_;
  WATCHDOG_FAULT WATCHDOG_FAULT_;
  BAT_FAULT BAT_FAULT_;
  CHRG_FAULT CHRG_FAULT_;
  BQ25896(){};
